...
async def start(message: types.Message, state: FSMContext):
    ...
    if entry == 'qr_1':
        ...
        if team:
            ...
            await message.answer(
                'Готовы к зимнему драйву на Снежном Фесте Абсолют Банка?'
                "Сегодня мы заряжаем праздничное настроение! ❄️⛸️"
                "Ваша миссия — пройти серию заданий, разгадать зимние загадки и собрать максимум впечатлений."
                "Под каждым QR кодом вас ждет блок уникальных заданий, "
                "если вы полностью выполните все задания из одного блока "
                "на 100% правильно, то вы сможете получить 1 жетон, ищите далее QR коды и получайте больше жетонов."
                "Берите смартфоны, сканируйте QR-код и отправляйтесь в ледяное приключение, "
                "полное азарта новогоднего волшебства."
                "Получить жетоны можно у эльфа на информационной стойке, "
                "там же вы можете задать любые вопросы по прохождению квеста."
                "Снежный Фест Абсолют Банка начинается!"
                "Прежде чем начать, необходимо пройти регистрацию:"
            )
            await message.answer(
                'Укажите ФИО:'
            )
